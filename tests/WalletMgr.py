...
class WalletMgr(object):
    __walletLogOutFile="test_kagrd_out.log"
    __walletLogErrFile="test_kagrd_err.log"
    __walletDataDir="test_wallet_0"
    __MaxPort=9999

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(kagrd) process; False=Manage launch process externally.
    def __init__(self, walletd, nodagrPort=8888, nodagrHost="localhost", port=9899, host="localhost"):
        self.walletd=walletd
        self.nodagrPort=nodagrPort
        self.nodagrHost=nodagrHost
        self.port=port
        self.host=host
        self.wallets={}
    ...
    def getArgs(self):
        return " --url http://%s:%d%s %s" % (self.nodagrHost, self.nodagrPort, self.getWalletEndpointArgs(), Utils.MiscAgrClientArgs)
    ...
    def findAvailablePort(self):
        for i in range(WalletMgr.__MaxPort):
            ...
            if Utils.arePortsAvailable(port):
                return port
            if Utils.Debug: Utils.Print("Port %d not available for %s" % (port, Utils.AgrWalletPath))

        Utils.errorExit("Failed to find free port to use for %s" % (Utils.AgrWalletPath))

    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch kagrd")
            return False
        ...
        pgrepCmd=Utils.pgrepCmd(Utils.AgrWalletName)
        if Utils.Debug:
            portTaken=False
            if self.isLocal():
            ...
            if psOut or portTaken:
                ...
                Utils.Print("Launching %s, note similar processes running. %s" % (Utils.AgrWalletName, statusMsg))

        cmd="%s --data-dir %s --config-dir %s --http-server-address=%s:%d --verbose-http-errors" % (
            Utils.AgrWalletPath, WalletMgr.__walletDataDir, WalletMgr.__walletDataDir, self.host, self.port)
        ...
        # Give kagrd time to warm up
        time.sleep(2)

        try:
            if Utils.Debug: Utils.Print("Checking if %s launched. %s" % (Utils.AgrWalletName, pgrepCmd))
            psOut=Utils.checkOutput(pgrepCmd.split())
            if Utils.Debug: Utils.Print("Launched %s. {%s}" % (Utils.AgrWalletName, psOut))
    ...
    def create(self, name, accounts=None, exitOnError=True):
        ...
        cmd="%s %s %s --name %s --to-console" % (Utils.AgrClientPath, self.getArgs(), cmdDesc, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=None
        maxRetryCount=4
        retryCount=0
    ...
    def importKey(self, account, wallet, ignoreDupKeyWarning=False):
        ...
        cmd="%s %s wallet import --name %s --private-key %s" % (
            Utils.AgrClientPath, self.getArgs(), wallet.name, account.ownerPrivateKey)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
        ...
        if account.activePrivateKey is None:
            ...
            cmd="%s %s wallet import --name %s  --private-key %s" % (
                Utils.AgrClientPath, self.getArgs(), wallet.name, account.activePrivateKey)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                Utils.checkOutput(cmd.split())
            except subprocess.CalledProcessError as ex:
    ...
    def lockWallet(self, wallet):
        cmd="%s %s wallet lock --name %s" % (Utils.AgrClientPath, self.getArgs(), wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def unlockWallet(self, wallet):
        cmd="%s %s wallet unlock --name %s" % (Utils.AgrClientPath, self.getArgs(), wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def lockAllWallets(self):
        cmd="%s %s wallet lock_all" % (Utils.AgrClientPath, self.getArgs())
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
    ...
    def getOpenWallets(self):
        ...
        cmd="%s %s wallet list" % (Utils.AgrClientPath, self.getArgs())
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=None
        try:
            retStr=Utils.checkOutput(cmd.split())
    ...
    def getKeys(self, wallet):
        ...
        p = re.compile(r'\n\s+\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet private_keys --name %s --password %s " % (Utils.AgrClientPath, self.getArgs(), wallet.name, wallet.password)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=None
        try:
            retStr=Utils.checkOutput(cmd.split())
    ...
    def killall(self, allInstances=False):
        """Kill kagr instances. allInstances will kill all kagr instances running on the system."""
        if self.__walletPid:
            Utils.Print("Killing wallet manager process %d" % (self.__walletPid))
            os.kill(self.__walletPid, signal.SIGKILL)

        if allInstances:
            cmd="pkill -9 %s" % (Utils.AgrWalletName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
