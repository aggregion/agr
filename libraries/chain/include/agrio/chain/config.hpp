...
namespace agrio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(agrio);
const static uint64_t null_account_name      = N(agrio.null);
const static uint64_t producers_account_name = N(agrio.prods);
...
const static uint64_t agrio_auth_scope       = N(agrio.auth);
const static uint64_t agrio_all_scope        = N(agrio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t agrio_any_name = N(agrio.any);
const static uint64_t agrio_code_name = N(agrio.code);
...
const static agrio::chain::wasm_interface::vm_type default_wasm_runtime = agrio::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace agrio::chain::config

constexpr uint64_t AGR_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / agrio::chain::config::percent_100;
}

template<typename Number>
Number AGR_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + agrio::chain::config::percent_100 - agrio::chain::config::percent_1)  / agrio::chain::config::percent_100;
}
