# AGR Testnet
To date, all work done to experiment with the AGR blockchain has been performed using a single instance of agrd hosting all 21 block producers. While this is a perfectly valid solution for validating features of the blockchain, developing new contracts, or whatever, it does not scale. Nor does it expose the sort of issues raised when contract and block data must be shared across multiple instances. Providing the ability to scale involves deploying multiple agrd nodes across many hosts and lining then into a peer-to-peer (p2p) network. Composing this network involves tailoring and distributing configuration files, coordinating starts and stops and other tasks.
...
Before getting into the details of the AGR testnet, lets clarify some terms. In this document I use the terms "host" and "machine" fairly interchangeably. A host generally boils down to a single IP address, although in practice it could have more.

The next term is "node." A node is an instance of the agrd executable configured to serve as 0 or more producers. There is not a one-to-one mapping between nodes and hosts, a host may serve more than one node, but one node cannot span more than one host. 
...
Finally there is the idea of distributed networks that involve remote hosts. These may be hosts on which you may not have direct access for starting and stopping agrd instances, but with whom you may wish to collaborate for setting up a decentralized testnet.
...
Note that the Launcher will not push instances of agrd to the remote hosts, you must prepare the various test network hosts separately.
...
#### Star network
![](https://github.com/aggregion/agr/raw/master/star.png)
...
#### Mesh network
![](https://github.com/aggregion/agr/raw/master/mesh.png)
...
To address the complexity implied by distributing multiple agrd nodes across a LAN or a wider network, the launcher application was created. 

Based on a handful of command line arguments the Launcher is able to compose per-node configuration files, distribute these files securely amongst the peer hosts, then start up the multiple instances of agrd.

Agrd instances started this way have their output logged in individual text files. Finally the launcher application is also able to shut down some or all of the test network. 

## Running the Launcher application

The launcher program is used to configure and deploy producing and non-producing agrd nodes that talk to each other using configured routes. The configuration for each node is stored in separate directories, permitting multiple nodes to be active on the same host, assuming the machine has sufficient memory and disk space for multiple agrd instances. The launcher makes use of multiple configuration sources in order to deploy a testnet. A handful of command line arguments can be used to set up simple local networks. 
...
launcher command line arguments:
  ...
  -d [ --delay ] arg (=0)               number of seconds to wait before starting the next node. Used to simulate a person keying in a series of individual agrd startup command lines.
  ...
  -o [ --output ] arg                   save a copy of the generated topology 
                                        in this file
  --skip-signature                      AGRD does not require transaction 
                                        signatures.
...
In this mode, the launcher does not activate any agrd instances, it produces a file of the given filename. This file is a JSON formatted template that provides an easy means of 
...
```
  "nodes": [[
      "testnet_0",{
        ...
        "ssh_identity": "",
        "ssh_args": "",
        "agr_root_dir": "/home/phil/blockchain/agr",
        "data_dir": "tn_data_0",
        "hostname": "remoteserv",
        ...
        "filesize": 8192,
        "keys": [{
            "public_key": "AGR8b2ojDi7mMWQxE3W6Fa1dr9bPfe5NTpRmyqBHECLNeFrdZHZrq",
            "wif_private_key": "5Hqh8bdtEjh9MFeEKzRTLX9pn68qoc17QfTz6JB8Pn59VojjVsS"
          }
        ],
        "peers": [
          "testnet_1",
...
| nodes         | a collection of descriptors defining the agrd instances used to assemble this testnet. The names used as keys in this collection are also aliases used within as placeholders for peer nodes.
...
| agr_root_dir       | specifies the directory into which all agrd artifacts are based. This is required for any hosts that are not the local host.
...
It is also necessary to provision the server by at least copying the agrd executable, and the genesis.json files to their appropriate locations relative to some named AGR root directory. For example, I defined the AGR root to be `/home/phil/blockchain/agr`. When run, the launcher will run through a variety of shell commands using ssh and finally using scp to copy a config.ini file to the appropriate data directory on the remote.
...
| :------------ | :----------
| config.ini    | The agrd configuration file.
| agrd.pid      | The process ID of the running agrd instance.
| blockchain/*  | The blockchain backing store
| blocks/*      | The blockchain log store
| stderr.txt    | The cerr output from agrd.
| stdout.txt    | The cout output from agrd.
