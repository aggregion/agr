...
namespace agrio { namespace testing {
  ...
  void base_tester::set_transaction_headers( transaction& trx, uint32_t expiration, uint32_t delay_sec ) const {
   ...
   void base_tester::set_before_preactivate_bios_contract() {
      set_code(config::system_account_name, contracts::before_preactivate_agrio_bios_wasm());
      set_abi(config::system_account_name, contracts::before_preactivate_agrio_bios_abi().data());
   }
