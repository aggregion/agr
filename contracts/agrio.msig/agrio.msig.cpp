...
void multisig::propose() {
   ...
   agrio_assert( trx_header.expiration >= agrio::time_point_sec(now()), "transaction expired" );
   //agrio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   agrio_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      agrio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < agrio::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
AGRIO_ABI( agrio::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
