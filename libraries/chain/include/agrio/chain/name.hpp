...
#define N(X) agrio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<agrio::chain::name> : private hash<uint64_t> {
      typedef agrio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const agrio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, agrio::chain::name& check);
} // fc


FC_REFLECT( agrio::chain::name, (value) )
