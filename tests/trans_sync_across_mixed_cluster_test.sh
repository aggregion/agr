...
killAll()
{
  programs/agrio-launcher/agrio-launcher -k 15
}
...
getHeadBlockNum()
{
  INFO="$(programs/clagr/clagr get info)"
  verifyErrorCode "clagr get info"
...
INITA_PRV_KEY="5Hqh8bdtEjh9MFeEKzRTLX9pn68qoc17QfTz6JB8Pn59VojjVsS"

# cleanup from last run
cleanup

# stand up nodagr cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodagr \"--plugin agrio::wallet_api_plugin\" $launcherOpts
programs/agrio-launcher/agrio-launcher --nodagr "--plugin agrio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clagr/clagr --port $port2 get block 1)"
    verifyErrorCode "clagr get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clagr/clagr create key)"
verifyErrorCode "clagr create key"
...
PASSWORD_INITA="$(programs/clagr/clagr wallet create --name inita)"
verifyErrorCode "clagr wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clagr/clagr wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clagr wallet import"
programs/clagr/clagr wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clagr wallet import"
programs/clagr/clagr wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clagr wallet import"
programs/clagr/clagr wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clagr wallet import"
...
ACCOUNT_INFO="$(programs/clagr/clagr create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clagr create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clagr/clagr get account testera)"
verifyErrorCode "clagr get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clagr/clagr transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clagr transfer"
   ...
   TRANS_INFO="$(programs/clagr/clagr --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clagr get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
