...
std::string wallet_manager::create(const std::string& name) {
   ...
   if (fc::exists(wallet_filename)) {
      AGR_THROW(chain::wallet_exist_exception, "Wallet with name: '${n}' already exists at ${path}", ("n", name)("path",fc::path(wallet_filename)));
   }
...
void wallet_manager::open(const std::string& name) {
   ...
   if (!wallet->load_wallet_file()) {
      AGR_THROW(chain::wallet_nonexistent_exception, "Unable to open file: ${f}", ("f", wallet_filename.string()));
   }
...
map<public_key_type,private_key_type> wallet_manager::list_keys(const string& name, const string& pw) {
   ...
   if (wallets.count(name) == 0)
      AGR_THROW(chain::wallet_nonexistent_exception, "Wallet not found: ${w}", ("w", name));
   auto& w = wallets.at(name);
   if (w->is_locked())
      AGR_THROW(chain::wallet_locked_exception, "Wallet is locked: ${w}", ("w", name));
...
void wallet_manager::unlock(const std::string& name, const std::string& password) {
   ...
   auto& w = wallets.at(name);
   if (!w->is_locked()) {
      AGR_THROW(chain::wallet_unlocked_exception, "Wallet is already unlocked: ${w}", ("w", name));
      return;
   }
   w->unlock(password);
}
...
wallet_manager::sign_transaction(const chain::signed_transaction& txn, const flat_set<public_key_type>& keys, const chain::chain_id_type& id) {
   ...
   for (const auto& pk : keys) {
      ...
      if (!found) {
         AGR_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", pk));
      }
   }
...
wallet_manager::sign_digest(const chain::digest_type& digest, const public_key_type& key) {
   ...
   AGR_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", key));
}
...
void wallet_manager::own_and_use_wallet(const string& name, std::unique_ptr<wallet_api>&& wallet) {
   if(wallets.find(name) != wallets.end())
      AGR_THROW(wallet_exception, "Tried to use wallet name that already exists.");
   wallets.emplace(name, std::move(wallet));
}
   ...
   t->async_wait([t, this](const boost::system::error_code& /*ec*/)
   {
      ...
      if(ec != boost::system::error_code()) {
         if(rc.type() == bfs::file_not_found) {
            appbase::app().quit();
            AGR_THROW(wallet_exception, "Lock file removed while kagrd still running.  Terminating.");
         }
      }
...
void wallet_manager::initialize_lock() {
   ...
   if(!wallet_dir_lock->try_lock()) {
      ...
      AGR_THROW(wallet_exception, "Failed to lock access to wallet directory; is another kagrd running?");
   }
