...
void copy_data(char* data, size_t data_len, agrio::vector<char>& data_out) {
...
void test_transaction::send_action() {
   ...
   action act(agrio::vector<permission_level>{{N(testapi), N(active)}}, test_action);
   act.send();
}
...
void test_transaction::assert_false_error_handler(const agrio::transaction& dtrx) {
   ...
   agrio_assert(dtrx.actions[0].account == N(testapi), "transaction has wrong code");
   agrio_assert(dtrx.actions[0].name == WASM_TEST_ACTION("test_action", "assert_false"), "transaction has wrong name");
   agrio_assert(dtrx.actions[0].authorization.size() == 1, "action should only have one authorization");
   agrio_assert(dtrx.actions[0].authorization[0].actor == N(testapi), "action's authorization has wrong actor");
   agrio_assert(dtrx.actions[0].authorization[0].permission == N(active), "action's authorization has wrong permission");
}
...
void test_transaction::new_feature() {
   agrio_assert(false == is_feature_active((int64_t)N(newfeature)), "we should not have new features unless hardfork");
}
