...
def stageScenario(stagedNodeInfos):
    ...
    for stagedNodeInfo in stagedNodeInfos:
        configPath=os.path.join(stagingDir, "etc/agrio/node_%02d" % (count))
...
killAgrInstances= not args.dont-kill
...
def myTest(transWillEnterBlock):
    ...
    try:
        ...
        if cluster.launch(pnodes, total_nodes, topo, delay) is False:
            error("Failed to stand up agr cluster.")
            return False
        ...
        if walletMgr.launch() is False:
            error("Failed to stand up agr walletd.")
            return False
