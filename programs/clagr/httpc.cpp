...
namespace agrio { namespace client { namespace http {
   ...
   resolved_url resolve_url( const http_context& context, const parsed_url& url ) {
      ...
      if (ec) {
         AGR_THROW(fail_to_resolve_host, "Error resolving \"${server}:${url}\" : ${m}", ("server", url.server)("port",url.port)("m",ec.message()));
      }
...
#elif defined( _WIN32 )
         AGR_THROW(http_exception, "HTTPS on Windows not supported");
#else
