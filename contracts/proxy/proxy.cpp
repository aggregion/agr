...
namespace proxy {
   ...
   void apply_transfer(uint64_t receiver, account_name /* code */, const T& transfer) {
      ...
      agrio_assert(get_res, "Attempting to use unconfigured proxy");
      if (transfer.from == self) {
         agrio_assert(transfer.to == code_config.owner,  "proxy may only pay its owner" );
      } else {
         agrio_assert(transfer.to == self, "proxy is not involved in this transfer");
         ...
         transaction out;
         out.actions.emplace_back(permission_level{self, N(active)}, N(agrio.token), N(transfer), new_transfer);
   ...
   void apply_setowner(uint64_t receiver, set_owner params) {
      ...
      agrio::print("Setting owner to: ", name{params.owner}, " with delay: ", params.delay, "\n");
      configs::store(code_config, self);
   }

   template<size_t ...Args>
   void apply_onerror(uint64_t receiver, const onerror& error ) {
      agrio::print("starting onerror\n");
      const auto self = receiver;
      config code_config;
      agrio_assert(configs::get(code_config, self), "Attempting use of unconfigured proxy");

      auto id = code_config.next_id++;
      configs::store(code_config, self);

      agrio::print("Resending Transaction: ", error.sender_id, " as ", id, "\n");
...
extern "C" {
    ...
    void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
      ...
      } else if( code == N(agrio.token) ) {
         if( action == N(transfer) ) {
            apply_transfer(receiver, code, unpack_action_data<agrio::token::transfer_args>());
         }
