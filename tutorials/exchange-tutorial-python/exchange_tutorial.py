...
def monitor_exchange():
    ...
    results = clagr('get actions tokenxchange {} 0 -j'.format(action_num))
...
def transfer(to, quantity):
    if quantity[:-4] != ' SYS':
        quantity += ' SYS'
    results = clagr('transfer tokenxchange {} "{}" {} -j'.format(to, quantity, KEY_TO_INTERNAL_ACCOUNT))
