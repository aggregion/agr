...
namespace agrio { namespace chain { namespace webassembly { namespace wabt_runtime {
...
struct wabt_apply_instance_vars {
   ...
   char* get_validated_pointer(uint32_t offset, uint32_t size) {
      AGR_ASSERT(memory, wasm_execution_error, "access violation");
      AGR_ASSERT(offset + size <= memory->data.size() && offset + size >= offset, wasm_execution_error, "access violation");
...
class wabt_runtime : public agrio::chain::wasm_runtime_interface {
   public:
      wabt_runtime();
...
#define _REGISTER_WABT_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static agrio::chain::webassembly::wabt_runtime::intrinsic_registrator _INTRINSIC_NAME(__wabt_intrinsic_fn, __COUNTER__) (\
      MOD,\
      NAME,\
      agrio::chain::webassembly::wabt_runtime::wabt_function_type_provider<WASM_SIG>::type(),\
      agrio::chain::webassembly::wabt_runtime::intrinsic_function_invoker_wrapper<SIG>::type::fn<&CLS::METHOD>()\
   );\

} } } }// agrio::chain::webassembly::wabt_runtime
