...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="agrio-ignition-wd/bootlog.txt"
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraNodagrArgs=None, useBiosBootFile=True, specificExtraNodagrArgs=None, onlySetProds=False,
        ...
        extraNodagrArgs: string of arguments to pass through to each nodoes instance (via --nodagr flag on launcher)
        ...
        specificExtraNodagrArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodagr flags on launcher), example: { "5" : "--plugin agrio::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the agrio.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodagr default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.AgrLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodagrArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
        ...
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodagrArgs += " --plugin agrio::producer_api_plugin"

        if nodagrArgs:
            cmdArr.append("--nodagr")
            cmdArr.append(nodagrArgs)

        if specificExtraNodagrArgs is not None:
            assert(isinstance(specificExtraNodagrArgs, dict))
            for nodeNum,arg in specificExtraNodagrArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodagr")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodagr")
                cmdArr.append(path)
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.AgrServerName, startedNodes, len(nodes)))
            return False
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by agrio-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/agrio.bios",
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/agrio.bios"
        ...
        if not self.walletMgr.importKey(agrioAccount, ignWallet):
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"agrio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init agrio transfer")
            opts="--permission agrio@active"
            if name != "agrio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from agrio.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (agrioAccount.name, CORE_SYMBOL)
        ...
        opts="--permission %s@active" % (agrioAccount.name)
        trans=biosNode.pushMessage(agrioAccount.name, action, data, opts)
        Utils.Print("Cluster bootstrap done.")

        return biosNode

    @staticmethod
    def pgrepAgrServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.AgrServerName)
    ...
    @staticmethod
    def pgrepAgrServerPattern(nodeInstance):
    ...
    def discoverLocalNodes(self, totalNodes, timeout=None):
        ...
        psOut=Cluster.pgrepAgrServers(timeout)
        if psOut is None:
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepAgrServerPattern(nodeNum)
    ...
    def discoverBiosNode(self, timeout=None):
        psOut=Cluster.pgrepAgrServers(timeout=timeout)
        pattern=Cluster.pgrepAgrServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.AgrServerName, pattern))
    ...
    def relaunchAgrInstances(self, cachePopen=False):
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodagr instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodagr instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
