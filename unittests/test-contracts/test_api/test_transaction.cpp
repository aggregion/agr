...
struct test_action_action {
   static agrio::name get_account() {
      return agrio::name{ACCOUNT};
   }

   static agrio::name get_name() {
      return agrio::name{NAME};
   }
...
void test_transaction::send_action_large() {
   ...
   act.send();
   agrio_assert( false, "send_message_large() should've thrown an error" );
}
...
void test_transaction::test_tapos_block_prefix() {
   ...
   agrio_assert( tbp == tapos_block_prefix(), "tapos_block_prefix does not match" );
}
...
void test_transaction::test_tapos_block_num() {
   ...
   agrio_assert( tbn == tapos_block_num(), "tapos_block_num does not match" );
}
...
void test_transaction::test_read_transaction() {
   ...
   agrio_assert( size == read, "read_transaction failed");
   h = agrio::sha256(buf, read);
   print(h);
}
...
void test_transaction::test_transaction_size() {
   ...
   agrio_assert( trans_size == transaction_size(), "transaction size does not match" );
}
...
void test_transaction::send_transaction_empty( uint64_t receiver, uint64_t, uint64_t ) {
   ...
   agrio_assert( false, "send_transaction_empty() should've thrown an error" );
}
...
void test_transaction::assert_false_error_handler( const agrio::transaction& dtrx ) {
   agrio_assert( dtrx.actions.size() == 1, "transaction should only have one action" );
   agrio_assert( dtrx.actions[0].account == "testapi"_n, "transaction has wrong code" );
   agrio_assert( dtrx.actions[0].name.value == WASM_TEST_ACTION("test_action", "assert_false"), "transaction has wrong name" );
   agrio_assert( dtrx.actions[0].authorization.size() == 1, "action should only have one authorization" );
   agrio_assert( dtrx.actions[0].authorization[0].actor == "testapi"_n, "action's authorization has wrong actor" );
   agrio_assert( dtrx.actions[0].authorization[0].permission == "active"_n, "action's authorization has wrong permission" );
}
...
void test_transaction::send_transaction_large( uint64_t receiver, uint64_t, uint64_t ) {
   ...
   agrio_assert( false, "send_transaction_large() should've thrown an error" );
}
...
void test_transaction::cancel_deferred_transaction_success() {
   ...
   agrio_assert( (bool)r, "transaction was not found" );
}

void test_transaction::cancel_deferred_transaction_not_found() {
   using namespace agrio;
   auto r = cancel_deferred( 0xffffffffffffffff ); //use the same id (0) as in send_deferred_transaction
   agrio_assert( !r, "transaction was canceled, whild should not be found" );
}
...
void test_transaction::send_cf_action_fail() {
   ...
   act.send_context_free();
   agrio_assert( false, "send_cfa_action_fail() should've thrown an error" );
}

void test_transaction::stateful_api() {
   char buf[4] = {1};
   db_store_i64( agrio::name{"testtrans"}.value, agrio::name{"table"}.value, agrio::name{"testtrans"}.value, 0, buf, 4 );
}
...
void test_transaction::repeat_deferred_transaction( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   trx.send( sender_id, agrio::name{receiver} );
}
