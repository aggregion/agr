...
fastUnstakeSystem = './fast.refund/agrio.system/agrio.system.wasm'
    ...
    for a in accounts:
        ...
        if not key in keys:
            if len(keys) >= args.max_user_keys:
                break
            keys[key] = True
            run(args.clagr + 'wallet import --private-key ' + key)
...
def startNode(nodeIndex, account):
    ...
    cmd = (
        ...
        '    --plugin agrio::http_plugin'
        '    --plugin agrio::chain_api_plugin'
        '    --plugin agrio::producer_plugin' +
        otherOpts)
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.clagr + 'create account agrio ' + a + ' ' + args.public_key)
...
def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(args.clagr + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])

def listProducers():
    run(args.clagr + 'system listproducers')
...
def vote(b, e):
    for i in range(b, e):
        ...
        retry(args.clagr + 'system voteproducer prods ' + voter + ' ' + prods)

def claimRewards():
    table = getJsonOutput(args.clagr + 'get table agrio agrio producers -l 100')
    times = []
    for row in table['rows']:
        if row['unpaid_blocks'] and not row['last_claim_time']:
            times.append(getJsonOutput(args.clagr + 'system claimrewards -j ' + row['owner'])['processed']['elapsed'])
    print('Elapsed time for claimrewards:', times)

def proxyVotes(b, e):
    vote(firstProducer, firstProducer + 1)
    proxy = accounts[firstProducer]['name']
    retry(args.clagr + 'system regproxy ' + proxy)
    sleep(1.0)
    for i in range(b, e):
        voter = accounts[i]['name']
        retry(args.clagr + 'system voteproducer proxy ' + voter + ' ' + proxy)

def updateAuth(account, permission, parent, controller):
    run(args.clagr + 'push action agrio updateauth' + jsonArg({
        'account': account,
...
def resign(account, controller):
    ...
    sleep(1)
    run(args.clagr + 'get account ' + account)
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.clagr + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def msigApproveReplaceSystem(proposer, proposalName):
    for i in range(firstProducer, firstProducer + numProducers):
        run(args.clagr + 'multisig approve ' + proposer + ' ' + proposalName +
            jsonArg({'actor': accounts[i]['name'], 'permission': 'active'}) +
            '-p ' + accounts[i]['name'])

def msigExecReplaceSystem(proposer, proposalName):
    retry(args.clagr + 'multisig exec ' + proposer + ' ' + proposalName + ' -p ' + proposer)

def msigReplaceSystem():
    run(args.clagr + 'push action agrio buyrambytes' + jsonArg(['agrio', accounts[0]['name'], 200000]) + '-p agrio')
    sleep(1)
...
def produceNewAccounts():
    with open('newusers', 'w') as f:
        for i in range(120_000, 200_000):
            x = getOutput(args.clagr + 'create key --to-console')
...
def stepLog():
    run('tail -n 60 ' + args.nodes_dir + '00-agrio/stderr')
...
parser.add_argument('--symbol', metavar='', help="The agrio.system symbol", default='SYS')
...
parser.add_argument('-H', '--http-port', type=int, default=8000, metavar='', help='HTTP port for clagr')
