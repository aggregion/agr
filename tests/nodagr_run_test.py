...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.agrioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    transId=node.createInitializeAccount(currencyAccount, cluster.agrioAccount, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.agrioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.agrioAccount, waitForTransBlock=True, exitOnError=True)
    ...
    if not enableMongo:
        ...
        account=node.getAgrAccountFromDb(currencyAccount.name)
    ...
    account=node.getAgrAccount(defproduceraAccount.name, exitOnError=True)
