...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        const auto& agrio_msig_acc = get<account_metadata_object, by_name>(N(agrio.msig));
        BOOST_TEST(agrio_msig_acc.is_privileged() == true);
        const auto& agrio_token_acc = get<account_metadata_object, by_name>(N(agrio.token));
        BOOST_TEST(agrio_token_acc.is_privileged() == true);
