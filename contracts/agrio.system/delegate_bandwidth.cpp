...
namespace agriosystem {
    ...
    */
   typedef agrio::multi_index< N(userres), user_resources>      user_resources_table;
   typedef agrio::multi_index< N(delband), delegated_bandwidth> del_bandwidth_table;
   typedef agrio::multi_index< N(refunds), refund_request>      refunds_table;
   ...
   void system_contract::buyrambytes( account_name payer, account_name receiver, uint32_t bytes ) {
      ...
      auto tmp = *itr;
      auto agrout = tmp.convert( asset(bytes,S(0,RAM)), CORE_SYMBOL );
   ...
   void system_contract::buyram( account_name payer, account_name receiver, asset quant )
   {
      ...
      INLINE_ACTION_SENDER(agrio::token, transfer)( N(agrio.token), {payer,N(active)},
         { payer, N(agrio.ram), quant_after_fee, std::string("buy ram") } );

      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(agrio::token, transfer)( N(agrio.token), {payer,N(active)},
                                                       { payer, N(agrio.ramfee), fee, std::string("ram fee") } );
      }
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      INLINE_ACTION_SENDER(agrio::token, transfer)( N(agrio.token), {N(agrio.ram),N(active)},
                                                       { N(agrio.ram), account, asset(tokens_out), std::string("sell ram") } );

      auto fee = ( tokens_out.amount + 199 ) / 200; /// .5% fee (round up)
      // since tokens_out.amount was asserted to be at least 2 earlier, fee.amount < tokens_out.amount
      
      if( fee > 0 ) {
         INLINE_ACTION_SENDER(agrio::token, transfer)( N(agrio.token), {account,N(active)},
            { account, N(agrio.ramfee), asset(fee), std::string("sell ram fee") } );
      }
   }
   ...
   {
      require_auth( from );
      agrio_assert( stake_net_delta != asset(0) || stake_cpu_delta != asset(0), "should stake non-zero amount" );
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         agrio_assert( asset(0) <= itr->net_weight, "insufficient staked net bandwidth" );
         agrio_assert( asset(0) <= itr->cpu_weight, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         agrio_assert( asset(0) <= tot_itr->net_weight, "insufficient staked total net bandwidth" );
         agrio_assert( asset(0) <= tot_itr->cpu_weight, "insufficient staked total cpu bandwidth" );
      ...
      if ( N(agrio.stake) != source_stake_from ) { //for agrio both transfer and refund make no sense
         ...
         if( is_delegating_to_self || is_undelegating ) {
            if ( req != refunds_tbl.end() ) { //need to update refund
               ...
               agrio_assert( asset(0) <= req->net_amount, "negative net refund amount" ); //should never happen
               agrio_assert( asset(0) <= req->cpu_amount, "negative cpu refund amount" ); //should never happen
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(agrio::token, transfer)( N(agrio.token), {source_stake_from, N(active)},
               { source_stake_from, N(agrio.stake), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
   ...
   {
      agrio_assert( stake_cpu_quantity >= asset(0), "must stake a positive amount" );
      agrio_assert( stake_net_quantity >= asset(0), "must stake a positive amount" );
      agrio_assert( stake_net_quantity + stake_cpu_quantity > asset(0), "must stake a positive amount" );
   ...
   {
      agrio_assert( asset() <= unstake_cpu_quantity, "must unstake a positive amount" );
      agrio_assert( asset() <= unstake_net_quantity, "must unstake a positive amount" );
      agrio_assert( asset() < unstake_cpu_quantity + unstake_net_quantity, "must unstake a positive amount" );
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      agrio_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
      // Until now() becomes NOW, the fact that now() is the timestamp of the previous block could in theory
      // allow people to get their tokens earlier than the 3 day delay if the unstake happened immediately after many
      // consecutive missed blocks.

      INLINE_ACTION_SENDER(agrio::token, transfer)( N(agrio.token), {N(agrio.stake),N(active)},
                                                    { N(agrio.stake), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
