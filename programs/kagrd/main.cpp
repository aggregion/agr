...
#include <agrio/wallet_api_plugin/wallet_api_plugin.hpp>
...
int main(int argc, char** argv)
{
   try {
      ...
      http.add_handler("/v1/" + kagrd::config::key_store_executable_name + "/stop", [](string, string, url_response_callback cb) { cb(200, fc::variant(fc::variant_object())); std::raise(SIGTERM); } );
      app().startup();
      app().exec();
