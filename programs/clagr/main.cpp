...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->add_flag("--add-code", add_code, localized("[code] add '${code}' permission to specified permission authority", ("code", name(config::agrio_code_name))));
      permissions->add_flag("--remove-code", remove_code, localized("[code] remove '${code}' permission from specified permission authority", ("code", name(config::agrio_code_name))));

      add_standard_transaction_options(permissions, "account@active");

      permissions->set_callback([this] {
         AGRC_ASSERT( !(add_code && remove_code), "ERROR: Either --add-code or --remove-code can be set" );
         AGRC_ASSERT( (add_code ^ remove_code) || !authority_json_or_file.empty(), "ERROR: authority should be specified unless add or remove code permission" );
         ...
         if ( need_parent || need_auth ) {
            ...
            if ( need_auth ) {
               ...
               auto code_name = name(config::agrio_code_name);
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      createAccount->set_callback([this] {
            ...
            if( owner_key_str.find('@') != string::npos ) {
               ...
               } AGR_RETHROW_EXCEPTIONS( explained_exception, "Invalid owner permission level: ${permission}", ("permission", owner_key_str) )
            } else {
               try {
            ...
            if( active_key_str.empty() ) {
               ...
               } AGR_RETHROW_EXCEPTIONS( explained_exception, "Invalid active permission level: ${permission}", ("permission", active_key_str) )
            } else {
               try {
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so clagr can still work with old buggy nodagr versions
                               // Change to voter.value when clagr no longer needs to support nodagr versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<agrio::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when clagr no longer needs to support nodagr versions older than 1.5.0
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so clagr can still work with old buggy nodagr versions
                               // Change to newname.value when clagr no longer needs to support nodagr versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        AGR_ASSERT( !wasm.empty(), wasm_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   auto stopKagrd = wallet->add_subcommand("stop", localized("Stop kagrd."), false);
   ...
   review->set_callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so clagr can still work with old buggy nodagr versions
                                 // Change to name(proposal_name).value when clagr no longer needs to support nodagr versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when clagr no longer needs to support nodagr versions older than 1.5.0
      ...
      std::map<agrio::account_name, std::pair<fc::time_point, vector<decltype(all_approvals)::iterator>>>  provided_approvers;
      ...
      if( show_approvals_in_multisig_review ) {
         ...
         if( new_multisig ) {
            for( auto& a : provided_approvers ) {
               ...
               if( rows4.empty() || rows4[0].get_object()["account"].as<agrio::name>() != a.first ) {
                  continue;
               }
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "agrio.msig", "invalidate", variant_to_bin( N(agrio.msig), "invalidate", args ) }});
   });
