...
namespace agrio { namespace chain {
   ...
   void block_log::open(const fc::path& data_dir) {
      ...
      if (log_size) {
         ...
         AGR_ASSERT( version > 0, block_log_exception, "Block log was not setup properly with genesis information." );
         AGR_ASSERT( version == block_log::supported_version, block_log_unsupported_version,
   ...
   uint64_t block_log::append(const signed_block_ptr& b) {
      try {
         ...
         AGR_ASSERT(my->index_stream.tellp() == sizeof(uint64_t) * (b->block_num() - 1),
