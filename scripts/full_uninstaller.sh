...
if [ -d "/usr/local/include/agrio" ] || [ -d "$HOME/opt/agrio" ] || [ $FORCED == 1 ]; then # use force for running the script directly
   printf "\nAGRIO installation (AND DEPENDENCIES) already found...\n"
   if [ $1 == 0 ]; then
   ...
   case $ANSWER in
      1 | [Yy]* )
         ...
         if [ -d "$HOME/opt/agrio" ] || [[ $1 == "force-new" ]]; then
            ...
            rm -rf $HOME/opt/agrio
            rm -f $HOME/bin/agrio-launcher
            rm -rf $HOME/lib/cmake/agrios
            rm -rf $HOME/opt/llvm
            rm -f $HOME/opt/boost
            ...
            rm -f $HOME/bin/nodagr $HOME/bin/kagrd $HOME/bin/clagr $HOME/bin/ctest $HOME/bin/*cmake* $HOME/bin/cpack
            rm -rf $HOME/src/mongo*
         fi

         if [ -d "/usr/local/include/agrio" ] || [[ $1 == "force-old" ]]; then
            if [ "$(id -u)" -ne 0 ]; then
            ...
            rm -rf libbson-1.0 libmongoc-1.0 mongocxx bsoncxx appbase chainbase agrio.system agriolib fc libc++ musl secp256k* 2>/dev/null
            rm -rf agrio 2>/dev/null
            popd &> /dev/null
            pushd bin &> /dev/null
            rm clagr agrio-abigen agrio-applesedemo agrio-launcher agrio-s2wasm agrio-wast2wasm agriocpp kagrd nodagr 2>/dev/null
            popd &> /dev/null
            libraries=(
               libagrio_testing
               libagrio_chain
