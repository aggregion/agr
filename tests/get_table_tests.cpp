...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = N(agrio.token) )
{
...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, agrio::chain::asset::from_string("999.0000 SYS") );
   }
   produce_blocks(1);

   // iterate over scope
   agrio::chain_apis::read_only plugin(*(this->control), fc::microseconds::maximum());
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, agrio::chain::asset::from_string("10000.0000 SYS") );
   }
   produce_blocks(1);
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, agrio::chain::asset::from_string("9999.0000 AAA") );
   }
   produce_blocks(1);
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, agrio::chain::asset::from_string("7777.0000 CCC") );
   }
   produce_blocks(1);
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, agrio::chain::asset::from_string("8888.0000 BBB") );
   }
   produce_blocks(1);
