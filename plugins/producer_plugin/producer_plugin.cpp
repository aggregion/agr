...
{
   auto default_priv_key = private_key_type::regenerate<fc::ecc::private_key_shim>(fc::sha256::hash(std::string("aggregion")));
...
producer_plugin::snapshot_information producer_plugin::create_snapshot() const {
   ...
   AGR_ASSERT( !fc::is_regular_file(snapshot_path), snapshot_exists_exception,
...
void producer_plugin_impl::produce_block() {
   ...
   const auto& pbs = chain.pending_block_state();
   const auto& hbs = chain.head_block_state();
   AGR_ASSERT(pbs, missing_pending_block_state, "pending_block_state does not exist but it should, another plugin may have corrupted it");
