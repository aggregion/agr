...
namespace agrio { namespace client { namespace http {
   ...
   std::string do_txrx(T& socket, boost::asio::streambuf& request_buff, unsigned int& status_code) {
      ...
      if( response_content_length != -1 ) {
         ...
         AGR_ASSERT(!ec || ec == boost::asio::ssl::error::stream_truncated, http_exception, "Unable to read http response: ${err}", ("err",ec.message()));
      }
   ...
   parsed_url parse_url( const string& server_url ) {
      ...
      if(res.scheme != "http" && res.scheme != "https")
         AGR_THROW(fail_to_resolve_host, "Unrecognized URL scheme (${s}) in URL \"${u}\"", ("s", res.scheme)("u", server_url));
      if(res.server.empty())
         AGR_THROW(fail_to_resolve_host, "No server parsed from URL \"${u}\"", ("u", server_url));
      if(res.port.empty())
   ...
   resolved_url resolve_url( const http_context& context, const parsed_url& url ) {
      ...
      if (ec) {
         AGR_THROW(fail_to_resolve_host, "Error resolving \"${server}:${port}\" : ${m}", ("server", url.server)("port",url.port)("m",ec.message()));
      }
