...
class state_history_log {
 ...
 public:
   ...
   void write_entry(const state_history_log_header& header, const chain::block_id_type& prev_id, F write_payload) {
      AGR_ASSERT(_begin_block == _end_block || header.block_num <= _end_block, chain::plugin_exception,
      ...
      AGR_ASSERT(end == pos + sizeof(header) + header.payload_size, chain::plugin_exception,
 ...
 private:
   ...
   void open_log() {
      ...
      if (size >= sizeof(state_history_log_header)) {
         ...
         log.seekg(0);
         log.read((char*)&header, sizeof(header));
         AGR_ASSERT(header.version == 0 && sizeof(header) + header.payload_size + sizeof(uint64_t) <= size,
   ...
   void open_index() {
      ...
      while (pos < size) {
         state_history_log_header header;
         AGR_ASSERT(pos + sizeof(header) <= size, chain::plugin_exception, "corrupt ${name}.log (6)", ("name", name));
         log.seekg(pos);
         log.read((char*)&header, sizeof(header));
         uint64_t suffix_pos = pos + sizeof(header) + header.payload_size;
         uint64_t suffix;
         AGR_ASSERT(suffix_pos + sizeof(suffix) <= size, chain::plugin_exception, "corrupt ${name}.log (7)",
                    ("name", name));
