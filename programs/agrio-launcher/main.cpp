...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodagr does not require transaction signatures.")
    ("nodagr", bpo::value<string>(&agrd_extra_args), "forward nodagr command line argument(s) to each instance of nodagr, enclose arg(s) in quotes")
    ("specific-num", bpo::value<vector<uint>>()->composing(), "forward nodagr command line argument(s) (using \"--specific-nodagr\" flag) to this specific instance of nodagr. This parameter can be entered multiple times and requires a paired \"--specific-nodagr\" flag")
    ("specific-nodagr", bpo::value<vector<string>>()->composing(), "forward nodagr command line argument(s) to its paired specific instance of nodagr(using \"--specific-num\"), enclose arg(s) in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodagr instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if (vmap.count("specific-num")) {
    ...
    const auto specific_args = vmap["specific-nodagr"].as<vector<string>>();
    if (specific_nums.size() != specific_args.size()) {
      cerr << "ERROR: every specific-num argument must be paired with a specific-nodagr argument" << endl;
      exit (-1);
    }
...
launcher_def::launch (agrd_def &instance, string &gts) {
  ...
  string agrdcmd = "programs/nodagr/nodagr ";
