...
public:
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(agrio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   abi_serializer initialize_multisig() {
      abi_serializer msig_abi_ser;
      {
         ...
         BOOST_REQUIRE_EQUAL( success(), buyram( "agrio", "agrio.msig", core_from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "agrio", "alice1111111", core_from_string("650000000.0000"), "agrio" );
...
inline uint64_t M( const string& agr_str ) {
   return core_from_string( agr_str ).get_amount();
}
