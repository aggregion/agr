...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for agrio reqauth action with alice's spending key
   chain.link_authority("alice", "agrio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "agrio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with agrio reqauth
   chain.unlink_authority("alice", "agrio", "reqauth");
   ...
   // Link authority for any agrio action with alice's scud key
   chain.link_authority("alice", "agrio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "agrio", "first",  "reqauth");
...
try {
   ...
   // Creating account with agrio. prefix with privileged account
   chain.create_account("agrio.test1");

   // Creating account with agrio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("agrio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'agrio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "agrio", "agrio.any", "reqauth" );
   chain.link_authority( "bob", "agrio", "agrio.any", "reqauth" );

   /// this should succeed because agrio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = agrio::chain::resource_limits::resource_usage_object;
   using by_owner = agrio::chain::resource_limits::by_owner;
