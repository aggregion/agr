...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("nodagr", bpo::value<string>(&agrd_extra_args), "forward nodagr command line argument(s) to each instance of nodagr, enclose arg in quotes")
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "cd " + host.agrio_home + "; "
                 + "export AGRIO_HOME=" + host.agrio_home + string("; ")
                 + "export AGRIO_NODE=" + node_num + "; "
                 + "./scripts/agrio-tn_bounce.sh " + agrd_extra_args;
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.agrio_home + "; "
                 + "export AGRIO_HOME=" + host.agrio_home + "; "
                 + "export AGRIO_NODE=" + node_num + "; "
         + "export AGRIO_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/agrio-tn_down.sh";
