...
void test_db::primary_i64_general(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   // find
   {
      ...
      agrio_assert(itr_next == itr_next_expected && prim == N(allyson), "primary_i64_general - db_find_i64" );
      itr_next = db_next_i64(itr_next, &prim);
      itr_next_expected = db_find_i64(receiver, receiver, table1, N(bob));
      agrio_assert(itr_next == itr_next_expected && prim == N(bob), "primary_i64_general - db_next_i64");
   }
   ...
   // previous
   {
      ...
      agrio_assert(itr_prev == itr_prev_expected && prim == N(bob), "primary_i64_general - db_previous_i64");

      itr_prev = db_previous_i64(itr_prev, &prim);
      itr_prev_expected = db_find_i64(receiver, receiver, table1, N(allyson));
      agrio_assert(itr_prev == itr_prev_expected && prim == N(allyson), "primary_i64_general - db_previous_i64");

      itr_prev = db_previous_i64(itr_prev, &prim);
      itr_prev_expected = db_find_i64(receiver, receiver, table1, N(alice));
      agrio_assert(itr_prev == itr_prev_expected && prim == N(alice), "primary_i64_general - db_previous_i64");

      itr_prev = db_previous_i64(itr_prev, &prim);
      agrio_assert(itr_prev < 0 && prim == N(alice), "primary_i64_general - db_previous_i64");
   }
...
void test_db::primary_i64_lowerbound(uint64_t receiver, uint64_t code, uint64_t action)
{
...

   {
      ...
      agrio_assert(lb == db_find_i64(receiver, receiver, table, N(alice)), err.c_str());
   }
   {
      int lb = db_lowerbound_i64(receiver, receiver, table, N(billy));
      agrio_assert(lb == db_find_i64(receiver, receiver, table, N(bob)), err.c_str());
   }
   {
      int lb = db_lowerbound_i64(receiver, receiver, table, N(frank));
      agrio_assert(lb == db_find_i64(receiver, receiver, table, N(joe)), err.c_str());
   }
   {
...
void test_db::primary_i64_upperbound(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   const std::string err = "primary_i64_upperbound";
   {
      ...
      agrio_assert(ub == db_find_i64(receiver, receiver, table, N(allyson)), err.c_str());
   }
   {
      int ub = db_upperbound_i64(receiver, receiver, table, N(billy));
      agrio_assert(ub == db_find_i64(receiver, receiver, table, N(bob)), err.c_str());
   }
   {
      int ub = db_upperbound_i64(receiver, receiver, table, N(frank));
      agrio_assert(ub == db_find_i64(receiver, receiver, table, N(joe)), err.c_str());
   }
   {
...
void test_db::idx64_general(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   // find_primary
   {
      ...
      agrio_assert(itr >= 0 && sec == N(joe), "idx64_general - db_idx64_find_primary");
   ...
   // iterate forward starting with charlie
   {
      ...
      agrio_assert(itr >= 0 && sec == N(charlie), "idx64_general - db_idx64_find_primary");
      ...
      agrio_assert(itr_next == itr_next_expected && sec_next == N(emily), "idx64_general - db_idx64_next");
      ...
      agrio_assert(itr_next == itr_next_expected && sec_next == N(joe), "idx64_general - db_idx64_next");
   ...
   // iterate backward staring with second bob
   {
      ...
      agrio_assert(itr >= 0 && sec == N(bob), "idx64_general - db_idx64_find_primary");
      ...
      agrio_assert(itr_prev == itr_prev_expected && sec_prev == N(bob), "idx64_general - db_idx64_previous");
      ...
      agrio_assert(itr_prev == itr_prev_expected && sec_prev == N(allyson), "idx64_general - db_idx64_previous");

      itr_prev = db_idx64_previous(itr_prev, &prim_prev);
      agrio_assert(itr_prev >= 0 && prim_prev == 265, "idx64_general - db_idx64_previous");
      itr_prev_expected = db_idx64_find_primary(receiver, receiver, table, &sec_prev, prim_prev);
      agrio_assert(itr_prev == itr_prev_expected && sec_prev == N(alice), "idx64_general - db_idx64_previous");
...
void test_db::idx64_lowerbound(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   const std::string err = "idx64_lowerbound";
   {
      ...
      agrio_assert(lb_prim == ssn && lb_sec == N(alice), err.c_str());
   ...
   }
   {
      ...
      int lb = db_idx64_lowerbound(receiver, receiver, table, &lb_sec, &lb_prim);
      agrio_assert(lb_prim == ssn && lb_sec == N(bob), err.c_str());
      agrio_assert(lb == db_idx64_find_primary(receiver, receiver, table, &lb_sec, ssn), err.c_str());
   }
   {
      ...
      int lb = db_idx64_lowerbound(receiver, receiver, table, &lb_sec, &lb_prim);
      agrio_assert(lb_prim == ssn && lb_sec == N(joe), err.c_str());
      agrio_assert(lb == db_idx64_find_primary(receiver, receiver, table, &lb_sec, ssn), err.c_str());
   }
   {
      ...
      uint64_t lb_prim = 0;
      int lb = db_idx64_lowerbound(receiver, receiver, table, &lb_sec, &lb_prim);
      agrio_assert(lb_prim == 0 && lb_sec == N(kevin), err.c_str());
      agrio_assert(lb < 0, "");
   }
}
...
void test_db::idx64_upperbound(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   const std::string err = "idx64_upperbound";
   {
      ...
      agrio_assert(ub_prim == allyson_ssn && ub_sec == N(allyson), "");
   ...
   }
   {
      ...
      int ub = db_idx64_upperbound(receiver, receiver, table, &ub_sec, &ub_prim);
      agrio_assert(ub_prim == bob_ssn && ub_sec == N(bob), "");
   ...
   }
   {
      ...
      uint64_t ub_prim = 0;
      int ub = db_idx64_upperbound(receiver, receiver, table, &ub_sec, &ub_prim);
      agrio_assert(ub_prim == 0 && ub_sec == N(joe), err.c_str());
      agrio_assert(ub < 0, err.c_str());
   }
   {
      ...
      uint64_t ub_prim = 0;
      int ub = db_idx64_upperbound(receiver, receiver, table, &ub_sec, &ub_prim);
      agrio_assert(ub_prim == 0 && ub_sec == N(kevin), err.c_str());
      agrio_assert(ub < 0, err.c_str());
   }
}
...
{
   (void)code;(void)action;
   auto act = agrio::get_action(1, 0);
   auto ia = agrio::unpack<invalid_access_action>(act.data);
   ...
   if( ia.store ) {
      ...
      //agrio::print("test_invalid_access: expected ", ia.val, " and retrieved ", value, "\n");
      agrio_assert( value == ia.val, "test_invalid_access: value did not match" );
   }
}
...
void test_db::idx_double_nan_lookup_fail(uint64_t receiver, uint64_t, uint64_t) {
   auto act = agrio::get_action(1, 0);
   auto lookup_type = agrio::unpack<uint32_t>(act.data);
...
void test_db::misaligned_secondary_key256_tests(uint64_t /* receiver */, uint64_t, uint64_t) {
   auto key = agrio::key256::make_from_word_sequence<uint64_t>(0ULL, 0ULL, 0ULL, 42ULL);
   char* ptr = (char*)(&key);
   ptr += 1;
