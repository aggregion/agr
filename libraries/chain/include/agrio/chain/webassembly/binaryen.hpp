...
namespace agrio { namespace chain { namespace webassembly { namespace binaryen {
...
struct interpreter_interface : ModuleInstance::ExternalInterface {
   ...
   Literal callImport(Import *import, LiteralList &args) override
   {
      ...
      AGR_ASSERT(fn_iter != import_lut.end(), wasm_execution_error, "unknown import ${m}:${n}", ("m", import->module.c_str())("n", import->module.c_str()));
   ...
   {
      AGR_ASSERT(index < table.size(), wasm_execution_error, "callIndirect: bad pointer");
      auto* func = instance.wasm.getFunctionOrNull(table[index]);
      AGR_ASSERT(func, wasm_execution_error, "callIndirect: uninitialized element");
      AGR_ASSERT(func->params.size() == arguments.size(), wasm_execution_error, "callIndirect: bad # of arguments");

      for (size_t i = 0; i < func->params.size(); i++) {
         AGR_ASSERT(func->params[i] == arguments[i].type, wasm_execution_error, "callIndirect: bad argument type");
      }
      AGR_ASSERT(func->result == result, wasm_execution_error, "callIndirect: bad result type");
   ...
   void assert_memory_is_accessible(uint32_t offset, uint32_t size) {
      AGR_ASSERT(offset + size <= current_memory_size && offset + size >= offset,
...
class binaryen_runtime : public agrio::chain::wasm_runtime_interface {
   public:
      binaryen_runtime();
...
{
   AGR_ASSERT( length < INT_MAX/(uint32_t)sizeof(T), binaryen_exception, "length will overflow" );
...
inline auto convert_native_to_literal(const interpreter_interface* interface, char* ptr) {
   ...
   AGR_ASSERT(ptr >= base && ptr < top_of_memory, wasm_execution_error, "returning pointer not in linear memory");
...
struct intrinsic_invoker_impl<Ret, std::tuple<T &, Inputs...>> {
   ...
   static auto translate_one(interpreter_interface* interface, Inputs... rest, LiteralList& args, int offset) -> std::enable_if_t<std::is_const<U>::value, Ret> {
      ...
      uint32_t ptr = args.at((uint32_t)offset).geti32();
      AGR_ASSERT(ptr != 0, binaryen_exception, "references cannot be created for null pointers");
      T* base = array_ptr_impl<T>(interface, ptr, 1);
      if ( reinterpret_cast<uintptr_t>(base) % alignof(T) != 0 ) {
...
#define _REGISTER_BINARYEN_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static agrio::chain::webassembly::binaryen::intrinsic_registrator _INTRINSIC_NAME(__binaryen_intrinsic_fn, __COUNTER__) (\
      MOD "." NAME,\
      agrio::chain::webassembly::binaryen::intrinsic_function_invoker_wrapper<SIG>::type::fn<&CLS::METHOD>()\
   );\
