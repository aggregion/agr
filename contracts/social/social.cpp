...
/**
 *  The purpose of this contract is to implement something like Steem on AGR, this
...
void apply_social_post() {
   ...
   agrio_assert( current_context() == post.author, "cannot call from any other context" );
...
void apply_social_vote() {
   ...
   if( context == vote.author ) {
      ...
      agrio_assert( Db::get( vote.postid, post ) > 0, "unable to find post" );
      agrio_assert( now() - post.created < days(7), "cannot vote after 7 days" );
      ...
      agrio_assert( abs_vote <= vote_account.vote_power, "insufficient vote power" );
      post.votes += vote.vote_power;
      vote_account.vote_power -= abs_vote;
      vote_account.last_vote  = now();
      Db::store( "account", vote_account );
   } else {
      agrio_assert( false, "invalid context for execution of this vote" );
   }
}
