...
asset get_currency_balance(const TESTER& chain, account_name account) {
   return chain.get_currency_balance(N(agrio.token), symbol(SY(4,CUR)), account);
}

const std::string agrio_token = name(N(agrio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.create_account(N(agrio.token));
   ...
   chain.push_action(config::system_account_name, linkauth::get_name(), tester_account, fc::mutable_variant_object()
           ...
           ("requirement", "first"));
   chain.produce_blocks();
   chain.push_action(N(agrio.token), N(create), N(agrio.token), mutable_variant_object()
           ("issuer", agrio_token)
           ("maximum_supply", "9000000.0000 CUR")
   );


   chain.push_action(N(agrio.token), name("issue"), N(agrio.token), fc::mutable_variant_object()
           ("to",       agrio_token)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action(N(agrio.token), name("transfer"), N(agrio.token), fc::mutable_variant_object()
       ("from", agrio_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, N(agrio.token));
   ...
   trace = chain.push_action(N(agrio.token), name("transfer"), N(tester), fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, N(agrio.token));
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester));
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   // transfer from agrio.token to tester
   trace = chain.push_action(N(agrio.token), name("transfer"), N(agrio.token), fc::mutable_variant_object()
       ("from", "agrio.token")
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>(N(agrio.token));
   const auto abi = acnt.get_abi();
   chain::abi_serializer abis(abi, chain.abi_serializer_max_time);
   const auto a = chain.control->db().get<account_object,by_name>(N(agrio.token)).get_abi();
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action(N(agrio.token), name("transfer"), vector<permission_level>{{N(tester), N(first)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(agrio.token), name("transfer"), vector<permission_level>{{N(tester), N(second)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(agrio.token), name("transfer"), vector<permission_level>{{N(tester), config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
