...
asset to_asset( const string& code, const string& s ) {
   static map<agrio::chain::symbol_code, agrio::chain::symbol> cache;
...
void ensure_kagrd_running(CLI::App* app) {
    ...
    for (const auto& addr: resolved_url.resolved_addresses)
       if (local_port_used(addr, resolved_url.resolved_port))  // Hopefully taken by kagrd
          return;
...
struct create_account_subcommand {
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      createAccount->set_callback([this] {
            ...
            if (!simple) {
               if ( buy_ram_agr.empty() && buy_ram_bytes_in_kbytes == 0) {
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("amount", amount, localized("The amount of AGR to pay for RAM, or number of kbytes of RAM if --kbytes is set"))->required();
...
int main( int argc, char** argv ) {
   ...
   getTransaction->set_callback([&] {
      ...
      } AGR_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", transaction_id_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   sudo_exec->set_callback([&] {
      ...
      auto accountPermissions = get_account_permissions(tx_permission);
      if( accountPermissions.empty() ) {
         accountPermissions = vector<permission_level>{{executer, config::active_name}, {"agrio.sudo", config::active_name}};
      }
      ...
      send_actions({chain::action{accountPermissions, "agrio.sudo", "exec", variant_to_bin( N(agrio.sudo), N(exec), args ) }});
   });
