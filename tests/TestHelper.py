...
class TestHelper(object):
    ...
    def parse_args(includeArgs, applicationSpecificArgs=AppArgs()):
        ...
        if "--clean-run" in includeArgs:
            parser.add_argument("--clean-run", help="Kill all nodagr and kleos instances", action='store_true')
        if "--sanity-test" in includeArgs:
            parser.add_argument("--sanity-test", help="Validates nodagr and kleos are in path and can be started up.", action='store_true')
