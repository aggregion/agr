...
Execute a transaction while bypassing regular authorization checks (requires authorization of agrio.sudo which needs to be a privileged account).

### agrio.sudo::exec    executer trx
...
## 2. Installing the agrio.sudo contract

The agrio.sudo contract needs to be installed on a privileged account to function. It is recommended to use the account `agrio.sudo`.

First, the account `agrio.sudo` needs to be created. Since it has the restricted `agrio.` prefix, only a privileged account can create this account. So this guide will use the `agrio` account to create the `agrio.sudo` account. On typical live blockchain configurations, the `agrio` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `agrio.msig` contract to help coordinate the approvals of the proposed transaction that creates the `agrio.sudo` account.

The `agrio.sudo` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`agrio`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `agrio` account needs to have enough of the core system token (the `SYS` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `agrio` account.

This guide will be using clagr to carry out the process.

### 2.1 Create the agrio.sudo account

#### 2.1.1 Generate the transaction to create the agrio.sudo account

The transaction to create the `agrio.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clagr command to propose the transaction to the agrio.msig contract.
...
```
$ clagr system newaccount -s -j -d --transfer --stake-net "1.000 SYS" --stake-cpu "1.000 SYS" --buy-ram-kbytes 50 agrio agrio.sudo AGR8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
726964ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea305500c80000"} arg: {"code":"agrio","action":"buyrambytes","args":{"payer":"agrio","receiver":"agrio.sudo","bytes":51200}}
726967ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea3055102700000000000004535953000000001027000000000000045359530000000001"} arg: {"code":"agrio","action":"delegatebw","args":{"from":"agrio","receiver":"agrio.sudo","stake_net_quantity":"1.0000 SYS","stake_cpu_quantity":"1.0000 SYS","transfer":true}}
...
Fourth, generate a transaction containing the `agrio::setpriv` action which will make the `agrio.sudo` account privileged:
```
$ clagr push action -s -j -d agrio setpriv '{"account": "agrio.sudo", "is_priv": 1}' -p agrio > generated_setpriv_trx.json
...
Next, the action JSONs of the previously generated transactions will be used to construct a unified transaction which will eventually be proposed with the agrio.msig contract. A good way to get started is to make a copy of the generated_newaccount_trx.json file (call the copied file create_sudo_account_trx.json) and edit the first three fields so it looks something like the following:
```
...
#### 2.1.2 Propose the transaction to create the agrio.sudo account
...
```
$ clagr multisig propose_trx createsudo producer_permissions.json create_sudo_account_trx.json blkproducera
executed transaction: bf6aaa06b40e2a35491525cb11431efd2b5ac94e4a7a9c693c5bf0cfed942393  744 bytes  772 us
#    agrio.msig <= agrio.msig::propose          {"proposer":"blkproducera","proposal_name":"createsudo","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.3 Review and approve the transaction to create the agrio.sudo account
...
```
$ clagr multisig review blkproducera createsudo > create_sudo_account_trx_to_review.json
...
```
$ clagr multisig propose_trx -j -s -d createsudo '[]' create_sudo_account_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_create_sudo_trx_serialized.hex
...
```
$ clagr multisig approve blkproducera createsudo '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 03a907e2a3192aac0cd040c73db8273c9da7696dc7960de22b1a479ae5ee9f23  128 bytes  472 us
#    agrio.msig <= agrio.msig::approve          {"proposer":"blkproducera","proposal_name":"createsudo","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.4 Execute the transaction to create the agrio.sudo account
...
```
$ clagr multisig exec blkproducera createsudo blkproducera
executed transaction: 7ecc183b99915cc411f96dde7c35c3fe0df6e732507f272af3a039b706482e5a  160 bytes  850 us
#    agrio.msig <= agrio.msig::exec             {"proposer":"blkproducera","proposal_name":"createsudo","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```

Anyone can now verify that the `agrio.sudo` was created:
```
$ clagr get account agrio.sudo
privileged: true
permissions:
...
### 2.2 Deploy the agrio.sudo contract

#### 2.2.1  Generate the transaction to deploy the agrio.sudo contract

The transaction to deploy the contract to the `agrio.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clagr command to propose the transaction to the agrio.msig contract.

The easy way to generate this transaction is using clagr:
```
$ clagr set contract -s -j -d agrio.sudo contracts/agrio.sudo/ > deploy_sudo_contract_trx.json
Reading WAST/WASM from contracts/agrio.sudo/agrio.sudo.wasm...
...
#### 2.2.2 Propose the transaction to deploy the agrio.sudo contract
...
```
$ clagr multisig propose_trx deploysudo producer_permissions.json deploy_sudo_contract_trx.json blkproducera
executed transaction: 9e50dd40eba25583a657ee8114986a921d413b917002c8fb2d02e2d670f720a8  4312 bytes  871 us
#    agrio.msig <= agrio.msig::propose          {"proposer":"blkproducera","proposal_name":"deploysudo","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.3 Review and approve the transaction to deploy the agrio.sudo contract

Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.

The proposed transaction can be reviewed using the `clagr multisig review` command:
```
$ clagr multisig review blkproducera deploysudo > deploy_sudo_contract_trx_to_review.json
...
```
$ clagr multisig propose_trx -j -s -d deploysudo '[]' deploy_sudo_contract_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_deploy_sudo_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ clagr multisig approve blkproducera deploysudo '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: d1e424e05ee4d96eb079fcd5190dd0bf35eca8c27dd7231b59df8e464881abfd  128 bytes  483 us
#    agrio.msig <= agrio.msig::approve          {"proposer":"blkproducera","proposal_name":"deploysudo","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.4 Execute the transaction to create the agrio.sudo account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `agrio.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the agrio.msig contract).

```
$ clagr multisig exec blkproducera deploysudo blkproducera
executed transaction: e8da14c6f1fdc3255b5413adccfd0d89b18f832a4cc18c4324ea2beec6abd483  160 bytes  1877 us
#    agrio.msig <= agrio.msig::exec             {"proposer":"blkproducera","proposal_name":"deploysudo","executer":"blkproducera"}
```

Anyone can now verify that the `agrio.sudo` contract was deployed correctly.

```
$ clagr get code -a retrieved-agrio.sudo.abi agrio.sudo
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-agrio.sudo.abi
$ sha256sum contracts/agrio.sudo/agrio.sudo.wasm
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/agrio.sudo/agrio.sudo.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-agrio.sudo.abi, can then be compared to the original ABI of the contract (contracts/agrio.sudo/agrio.sudo.abi) to ensure they are semantically the same.

## 3. Using the agrio.sudo contract

### 3.1 Example: Updating owner authority of an arbitrary account

This example will demonstrate how to use the deployed agrio.sudo contract together with the agrio.msig contract to allow a greater than two-thirds supermajority of block producers of an AGRIO blockchain to change the owner authority of an arbitrary account. The example will use clagr: in particular, the `clagr multisig` command, the `clagr set account permission` sub-command, and the `clagr sudo exec` sub-command. However, the guide also demonstrates what to do if the `clagr sudo exec` sub-command is not available.
...
The next step is to generate the transaction containing the `agrio.sudo::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.

```
$ clagr sudo exec -s -j -d blkproducera update_alice_owner_trx.json > sudo_update_alice_owner_trx.json
```
...
If the `clagr sudo` command is not available, there is an alternative way to generate the above transaction. There is no need to continue reading the remaining of sub-section 3.1.1 if the sudo_update_alice_owner_trx.json file was already generated with content similar to the above using the `clagr sudo exec` sub-command method.
...
Then generate the template for the transaction containing the `agrio.sudo::exec` action:
```
$ clagr push action -s -j -d agrio.sudo exec '{"executer": "blkproducera", "trx": ""}' > sudo_update_alice_owner_trx.json
$ cat sudo_update_alice_owner_trx.json
{
...
```
$ clagr multisig propose_trx updatealice producer_permissions.json sudo_update_alice_owner_trx.json blkproducera
...
Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.
```
$ clagr multisig review blkproducera updatealice > sudo_update_alice_owner_trx_to_review.json
...
The approvers should go through the human-readable transaction output and make sure everything looks fine. However, due to a current limitation of nodagr/clagr, the JSONification of action payload data does not occur recursively. So while both the `hex_data` and the human-readable JSON `data` of the payload of the `agrio.sudo::exec` action is available in the output of the `clagr multisig review` command, only the hex data is available of the payload of the inner `agrio::updateauth` action. So it is not clear what the `updateauth` will actually do.

Furthermore, even if this usability issue was fixed in nodagr/clagr, there will still be cases where there is no sensible human-readable version of an action data payload within a transaction. An example of this is the proposed transaction in sub-section 2.2.3 which used the `agrio::setcode` action to set the contract code of the `agrio.sudo` account. The best thing to do in such situations is for the reviewer to compare the proposed transaction to one generated by them through a process they trust.
...
```
$ clagr multisig propose_trx -j -s -d updatealice '[]' sudo_update_alice_owner_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_sudo_update_alice_owner_trx_serialized.hex
