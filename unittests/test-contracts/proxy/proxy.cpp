...
proxy::proxy( agrio::name self, agrio::name first_receiver, agrio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      agrio::token::transfer_action a( "agrio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, agrio::ignore<std::vector<char>> ) {
